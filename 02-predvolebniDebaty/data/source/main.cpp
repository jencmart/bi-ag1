...
struct data
{
    ...
    {        // no childs
        ...
        // have both childrens - nikdy nenastane
    }
...
///*************************** REMOVE **********************************************************************************
    data BVSRemove(Node * node, int val)
    {
        ...
        else if(val < node->m_value ) // val < node
        {
            ...
            if(node->m_left)
                node->m_left->m_parent = node; // !! todo pozor ten node nemusi existovat uz !!!!
            sig = dat.m_sig;
            ...
            if(sig)
            {
                ...
                else if(node->m_leftDepth + node->m_rightDept == +2)
                {
                    ...
                    if(y->m_leftDepth + y->m_rightDept == +1 || y->m_leftDepth + y->m_rightDept == 0 ) // +1 aka. h+1 h h ->right rotation
                    {
